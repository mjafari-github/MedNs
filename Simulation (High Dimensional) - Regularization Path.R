##### SIMULATION #####
##### High Dimensional Setting - ONLY REGULARIZATION PATH RECORDED FOR PLOTTING
##### June 27, 2024
...
##### BEGIN REPLICATIONS
{
  ...
  for(b in 1:B){
    ...
    if(b %% 1 == 0){ print(paste("iteration", sep = " = ", b)) } # counts every 5 iterations
  }
  end = Sys.time()
...
#write.csv(avg.path, file = here::here("Results", paste("TEST1000HighDim-PATH-Case", case, "-tau", tau, ".csv", sep = "")))
