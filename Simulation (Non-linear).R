...
if(F){
  B = 10 # number of replications for simulation
  tau = 0.5 # quantile
  case = 1 # choose 1 through 3
}
...
ISE.poly <- function(coef, tau){
  ...
  sqErr.poly <- function(x){
    # true relationship
    if(case == 1){ true = 2.5 + sin(2*x) + 2*exp(-16*x^2) + qnorm(tau, 0, 1) }
    if(case == 2){ true = 2 + 2*cos(x) + exp(-4*x^2) + qexp(tau, 1) }
...
##### BEGIN REPLICATIONS
{
  ...
  for(b in 1:B){
    ...
    mean.val <- MeanReg.POLYvalidation(train.response = y.trainSRS, train.features = features.trainSRS,
                                            test.response = y.val, test.features = features.val,
                                            degrees = degrees)
    mean.model <- lm(y.trainSRS ~ poly(features.trainSRS, degree = degrees[which.min(mean.val$errors)]))
    mean.coefs <- coef(mean.model)
    
    ISE.mean[b] <- ISE.poly(mean.coefs, tau = tau)$ISE
    
    length(mean.coefs) <- D
    coef.mean[b,] <- mean.coefs
    
    # QR

    QR.val <- QR.POLYvalidation(train.response = y.trainSRS, train.features = features.trainSRS,
                                     test.response = y.val, test.features = features.val,
                                     tau = tau, degrees = degrees)
    #QR.model <- rq(y.trainSRS ~ poly(features.trainSRS, degree = degrees[which.min(QR.val$errors)]))
    #QR.coefs <- coef(QR.model)
    
    QR.model <- QR.POLY(y.trainSRS, features.trainSRS, tau=tau, degree = degrees[which.min(QR.val$errors)])
    QR.coefs <- QR.model
    
    ISE.QR[b] <- ISE.poly(QR.coefs, tau = tau)$ISE
    ...
    for(r in 1:ncol(rankers)){
      ...
      ## k=3
      if(tau <= 2/3 && tau >= 1/3){
        model.coef.3val <- MedNS.POLYvalidation(train.response = y.trainK3, train.features = features.trainK3,
                                                test.response = y.val, test.features = features.val,
                                                tau=tau, m=2, degrees = degrees)
        model.coef.3 <- MedNS.POLY(response = y.trainK3, features = features.trainK3,
                                         tau=tau, m=2, degree = degrees[which.min(model.coef.3val$errors)])
        
        ISE.K3[b, r] <- ISE.poly(model.coef.3, tau = tau)$ISE
        
        length(model.coef.3) <- D
        coef3 <- c(coef3, model.coef.3)
      }
      ## k=5
      if(tau <= 3/5 && tau >= 2/5){
        model.coef.5val <- MedNS.POLYvalidation(train.response = y.trainK5, train.features = features.trainK5,
                                                test.response = y.val, test.features = features.val,
                                                tau=tau, m=3, degrees = degrees)
        model.coef.5 <- MedNS.POLY(response = y.trainK5, features = features.trainK5,
                                         tau=tau, m=3, degree = degrees[which.min(model.coef.5val$errors)])
        
        ISE.K5[b, r] <- ISE.poly(model.coef.5, tau = tau)$ISE
        
        length(model.coef.5) <- D
        coef5 <- c(coef5, model.coef.5)
      }
      ## k=7
      if(tau <= 4/7 && tau >= 3/7){
        model.coef.7val <- MedNS.POLYvalidation(train.response = y.trainK7, train.features = features.trainK7,
                                                test.response = y.val, test.features = features.val,
                                                tau=tau, m=4, degrees = degrees)
        model.coef.7 <- MedNS.POLY(response = y.trainK7, features = features.trainK7,
                                         tau=tau, m=4, degree = degrees[which.min(model.coef.7val$errors)])
        
        ISE.K7[b, r] <- ISE.poly(model.coef.7, tau = tau)$ISE
        
        length(model.coef.7) <- D
        coef7 <- c(coef7, model.coef.7)
      }
      
      ##### SRS from induced population Approach
      
      val.3SRSinduced <- QR.POLYvalidation(train.response = y.trainK3, train.features = features.trainK3,
                                  test.response = y.val, test.features = features.val,
                                  tau=tau.k.imperfect[r, 1], degrees = degrees)
      #model.3SRSinduced <- rq(y.trainK3 ~ poly(features.trainK3, degree = degrees[which.min(val.3SRSinduced$errors)]))
      model.3SRSinduced <- QR.POLY(y.trainK3, features.trainK3, tau=tau, degree = degrees[which.min(val.3SRSinduced$errors)])
      
      
      val.5SRSinduced <- QR.POLYvalidation(train.response = y.trainK5, train.features = features.trainK5,
                                           test.response = y.val, test.features = features.val,
                                           tau=tau.k.imperfect[r, 2], degrees = degrees)
      #model.5SRSinduced <- rq(y.trainK5 ~ poly(features.trainK5, degree = degrees[which.min(val.5SRSinduced$errors)]))
      model.5SRSinduced <- QR.POLY(y.trainK5, features.trainK5, tau=tau, degree = degrees[which.min(val.5SRSinduced$errors)])
      
      val.7SRSinduced <- QR.POLYvalidation(train.response = y.trainK7, train.features = features.trainK7,
                                           test.response = y.val, test.features = features.val,
                                           tau=tau.k.imperfect[r, 3], degrees = degrees)
      model.7SRSinduced <- rq(y.trainK7 ~ poly(features.trainK7, degree = degrees[which.min(val.7SRSinduced$errors)]))
      model.7SRSinduced <- QR.POLY(y.trainK7, features.trainK7, tau=tau, degree = degrees[which.min(val.7SRSinduced$errors)])
      
      # record coefficients & error
      #coefs3 <- coef(model.3SRSinduced)
      #coefs5 <- coef(model.5SRSinduced)
      #coefs7 <- coef(model.7SRSinduced)
      
      coefs3 <- model.3SRSinduced
      coefs5 <- model.5SRSinduced
      coefs7 <- model.7SRSinduced
      
      ISE.K3SRSinduced[b, r] <- ISE.poly(coefs3, tau = tau)$ISE
      ISE.K5SRSinduced[b, r] <- ISE.poly(coefs5, tau = tau)$ISE
      ISE.K7SRSinduced[b, r] <- ISE.poly(coefs7, tau = tau)$ISE
      
      length(coefs3) <- length(coefs5) <- length(coefs7) <- D
      coef3SRSinduced <- c(coef3SRSinduced, coefs3)
      coef5SRSinduced <- c(coef5SRSinduced, coefs5)
      coef7SRSinduced <- c(coef7SRSinduced, coefs7)
    }
...
#write.csv(coef.all, file = here::here("Results", paste("1000Poly-Coef-Case", case, "-tau", tau, ".csv", sep = "")))
...
#write.csv(ISE, file = here::here("Results", paste("1000Poly-err-Case", case, "-tau", tau, ".csv", sep = "")))
